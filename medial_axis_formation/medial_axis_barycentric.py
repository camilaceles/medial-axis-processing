...
import copy
import trimesh
from utils import manifold_to_trimesh


class MedialAxisBarycentric:
    ...
    def __map_correspondences(self):
        ...
        trim = manifold_to_trimesh(self.mesh)
        prox_query = trimesh.proximity.ProximityQuery(trim)
        _, _, face_ids = prox_query.on_surface(self.inner_points.positions)

        triangles = trim.triangles[face_ids]
        barycentrics = trimesh.triangles.points_to_barycentric(triangles, self.inner_points.positions)
